...
net = caffe.Net( 'DeepModel.prototxt',
                'models/DeepModel_iter_222152.caffemodel',
                caffe.TEST)
import numpy as np
...
for t in range(iters):
    ...
    for i in range(0,batch_size):
        ...
        for j in range(0,J):
            ...
            gtx[j]=net.blobs['jointlabel'].data[i][NYU_joint[j] * 3]
            y[j]=net.blobs['DeepHandModelxyz'].data[i][NYU_joint[j] * 3 + 1]
            gty[j]=net.blobs['jointlabel'].data[i][NYU_joint[j] * 3 + 1]            
            z[j]=net.blobs['DeepHandModelxyz'].data[i][NYU_joint[j] * 3 + 2]
            gtz[j]=net.blobs['jointlabel'].data[i][NYU_joint[j] * 3 + 2]
