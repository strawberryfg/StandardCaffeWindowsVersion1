...
net = caffe.Net( 'DeepModel.prototxt',
                'models/DeepModel_iter_190000.caffemodel',
                caffe.TEST)
import numpy as np
...
for t in range(iters):
    ...
    for i in range(0,batch_size):
        ...
        if output_param_jt:
            ...
            z = np.zeros(31)            
            for j in range(0,31):
                x[j]=net.blobs['DeepHandModelxyz'].data[i][j * 3]                
                y[j]=net.blobs['DeepHandModelxyz'].data[i][j * 3 + 1]                
                z[j]=net.blobs['DeepHandModelxyz'].data[i][j * 3 + 2]

            for j in range(0,31):
                jt_31_file.write('{:f} {:f} {:f} '.format(x[j], y[j], z[j]))
            jt_31_file.write('\n')
